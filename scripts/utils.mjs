...
export const createModule = async (type, moduleName) => {
  ...
  createPlaywrightConfig(type, moduleName);
...
export const createTsConfig = (type, moduleName) => {
  ...
  if (type === "test") {
    ...
    tsConfigTmpl.include.push("playwright.config.ts");
  } else if (type === "lib") {
    tsConfigTmpl.references = [
      {
        path: "../types",
...
export const createPackageJson = (type, moduleName) => {
  ...
  if (type === "test") {
    packageJsonTmpl.scripts["test"] = "playwright test";
...
export const createPlaywrightConfig = (type, moduleName) => {
  if (type === "lib") return;
  console.log(`Creating playwright.config for '@${type}/${moduleName}'`);
  ...
  `;
  fs.writeFileSync(resolve(modulePath, "playwright.config.ts"), tmpl, "utf-8");
};
